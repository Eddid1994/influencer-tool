...
export default function BulkCreateEngagementsPage() {
  ...
  const createAllCampaigns = async () => {
    ...
    try {
      ...
      if (!user) {
        toast.error('You must be logged in to create engagements')
        return
      }

      // Prepare all engagements for batch insertion
      ...
      for (const assignment of assignments) {
        for (const influencerId of assignment.influencerIds) {
          ...
          const campaignName = `${brand?.name || 'Brand'} x ${influencer?.name || 'Influencer'} - ${now.toLocaleDateString()}`
          
          // Convert budget from euros to cents for storage
          const budgetInCents = parseNumberSafely(campaignTemplate.budget) ? Math.round(parseNumberSafely(campaignTemplate.budget)! * 100) : null
          ...
          engagementsToCreate.push({
            ...
            status: campaignTemplate.status === 'planned' ? 'negotiating' as const : 
                   campaignTemplate.status === 'active' ? 'active' as const :
                   campaignTemplate.status === 'completed' ? 'completed' as const : 'negotiating' as const,
            agreed_currency: 'EUR',
            created_by: user.id,
            // Campaign-specific fields (consolidated from old campaigns table)
            ...
            // Negotiation fields with sensible defaults
            negotiation_status: 'pending_outreach' as const,
            negotiation_data: {},
            negotiation_priority: 'medium' as const,
            tags: [campaignTemplate.channel, campaignTemplate.content_type].filter(Boolean)
          })
        }
      }

      // Batch insert all engagements
      const { data, error } = await supabase
        .from('engagements')
      ...
      if (error) {
        ...
        toast.error(`Failed to create engagements: ${error.message}`)
        return
      }

      const created = data?.length || 0
      toast.success(`Successfully created ${created} engagement${created !== 1 ? 's' : ''}`)
  ...
  return (
    <div className="container mx-auto py-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">Bulk Create Engagements</h1>
          <p className="text-gray-500">Select influencers, assign brands, and create multiple engagements at once</p>
        </div>
      </div>
      ...
      {!isLoadingData && step === 1 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Choose the brands you want to create engagements for</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-yellow-700">Each brand can work with multiple influencers. A separate engagement will be created for each brand-influencer combination.</p>
                  </div>
                </div>
              </div>
      ...
      {/* Step 3: Set Engagement Details & Create */}
      {!isLoadingData && step === 3 && (
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Step 3: Engagement Details</CardTitle>
              <CardDescription>Set default values for all engagements (you can edit individual engagements later)</CardDescription>
            </CardHeader>
            <CardContent>
              ...
              <div className="space-y-4">
                <div className="bg-gray-50 rounded-lg p-4">
                  <div className="text-2xl font-bold mb-2">
                    {assignments.reduce((sum, a) => sum + a.influencerIds.length, 0)} Engagements
                  </div>
