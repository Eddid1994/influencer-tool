...
with open(csv_file, 'r', encoding='utf-8') as f:
    ...
    for row in reader:
        ...
        if row['Influencer'] not in influencers:
            influencers.add(row['Influencer'])
            name = escape_string(row['Influencer'])
            handle = escape_string(row['Influencer'].lower())
            influencers_sql.append(f"('{name}', '{handle}', 'active', NOW(), NOW())")
        ...
        campaigns_sql.append({
            'brand': brand_name,
            'influencer': influencer_name,
            'values': f"""(
    (SELECT id FROM brands WHERE name = '{brand_name}' LIMIT 1),
    (SELECT id FROM influencers WHERE name = '{influencer_name}' LIMIT 1),
...
# Print influencers insert in batches
print("-- Inserting Influencers in batches --")
batch_size = 50
for i in range(0, len(influencers_sql), batch_size):
    batch = influencers_sql[i:i+batch_size]
    print(f"INSERT INTO influencers (name, instagram_handle, status, created_at, updated_at) VALUES")
    print(",\n".join(batch))
    print("ON CONFLICT (instagram_handle) DO UPDATE SET name = EXCLUDED.name;")
    print()

# Print campaigns insert
print("-- Inserting Campaigns --")
print(f"-- Total: {len(campaigns_sql)} campaigns --")
for i, campaign in enumerate(campaigns_sql[:10]):  # Print first 10 as examples
    print(f"-- Campaign {i+1}: {campaign['brand']} - {campaign['influencer']} --")
    print(f"""INSERT INTO campaigns (
